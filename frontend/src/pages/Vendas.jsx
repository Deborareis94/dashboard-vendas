...
export default function Vendas() {
    ...
    fetch('/api/vendas', {
      ...
      .then(venda => {
        ...
        fetch(`/api/produtos/${produto.id}`, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ ...produto, estoque: novoEstoque })
        });
      });
  ...
  return (
    <div className="space-y-6">
      ...
      <form onSubmit={registrarVenda} className="bg-gray-800 p-4 rounded-lg flex gap-3">
        <select
      ...
      <div className="flex justify-between items-center">
        ...
        <h3 className="text-lg font-semibold">
          {dataFiltro ? "Faturamento do dia" : "Faturamento total"}: R$ {faturamento.toFixed(2)}
        </h3>
      </div>
          ...
          <tbody>
            {vendasFiltradas.map(v => (
              <tr key={v.id} className="border-t border-gray-700">
                <td className="p-4">{v.produtoNome}</td>
                <td className="p-4">{v.quantidade}</td>
                <td className="p-4">R$ {Number(v.valor).toFixed(2)}</td>
                <td className="p-4">
